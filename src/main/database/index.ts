...
export function exportCSV(): string {
  ...
  const header = ['日期','类型','分类','金额','账户','备注','创建时间']
  const rows = records
    ...
    .map(r => {
      ...
      const typeLabel = r.type === 'income' ? '收入' : r.type === 'expense' ? '支出' : '转账'
      return [esc(r.date), esc(typeLabel), esc(cat ? cat.icon + ' ' + cat.name : ''), esc(r.amount.toFixed(2)), esc(acc?.name || ''), esc(r.note || ''), esc(r.createdAt || '')].join(',')
    })
